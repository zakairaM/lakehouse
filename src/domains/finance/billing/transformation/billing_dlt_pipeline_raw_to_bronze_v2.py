...
def create_bronze_dlt_table(entity_code, region, document_type, source_config, raw_path, schema_location, file_format, expectations):
    ...
    # -------------------------
    # 2️⃣ VALID VIEW (por entidade)
    # -------------------------
    #@dlt.view(
    #    name=valid_view_name,
    #    comment=f"Valid rows view for {entity_code}/{document_type} in {region}"
    #)
    #@dlt.expect_all_or_drop(expectations if expectations else {#"always_true": "true"})
    #def valid_view():
    #    return dlt.read(bronze_table_name)

    ## Registrar tabelas e views para a tabela global de erros
    #bronze_tables_with_valid_views.append((bronze_table_name, v#alid_view_name))

    #return bronze_table_name, valid_view_name


# --------------------------
# Tabela global de erros (agrega todas as entidades)
# --------------------------


    #@dlt.table(
    #    name=error_table_name,
    #    comment=f"Rows failing expectations for {entity_code}/#{document_type}",
    #    table_properties={"quality": "error"}
    #)
    #def entity_error_table():
    #    bronze_df = dlt.read(bronze_table_name)
    #    valid_df  = dlt.read(valid_view_name)
    #    errors_df = bronze_df.join(valid_df, ["_raw_record_hash"], #"leftanti")
    #    errors_df = errors_df.withColumn("_entity_code", F.lit(entity_code))
    #    errors_df = errors_df.withColumn("_document_type", F.lit#(document_type))
    #    errors_df = errors_df.withColumn("_error_reason", F.lit#("failed_expectations"))
    #    return errors_df
